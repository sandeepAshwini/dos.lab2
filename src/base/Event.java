...
public class Event implements Serializable, Runnable {
	...
	/**
	 * Generates a random number of players for the event. This number is
	 * greater than 3, since we have 3 places.
	 * 
	 */
	private void setScores() {
		Random rand = new Random();
		this.numberOfParticipants = rand.nextInt(RANGE_NUMBER_PARTICIPANTS)
	...
	/**
	 * Simulates the event. Run method simulates the event. The event consists
	 * of 5 legs and a random number of players. There is a constant interval
	 * between legs. All participant's scores are randomly initialized, and
	 * randomly updated in each leg.
	 * */
	public void run() {
		int count = 0;
	...
	/**
	 * Calls incrementScore on each participant. Scores can be accessed at any
	 * time by the server, hence the update is synchronized.
	 */
	...
	/**
	 * Returns the current scores. Again, updates may happen at any time, and
